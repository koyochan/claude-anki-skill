...
**裏面**:
- 字幕全体の日本語訳（1 行）
- **解説（1〜2 行）**: 文のニュアンス・文脈・なぜこの語法か・文化的背景・話者の意図など。語彙単位の意味ではなく **文レベルのコメント**。例: 「TED talk で AI の自律性を語る前置き」「皮肉のトーン」「比喩でビジネス基盤を指す」など。straightforward な文では省略可。
...
<div style="font-family: -apple-system, 'Helvetica Neue', sans-serif; line-height:1.6; text-align:left;">
  <!-- 字幕全体の日本語訳 -->
  <div style="font-weight:500; font-size:1.1em; margin-bottom:12px;">{JP_TRANSLATION}</div>

  <!-- 解説（文レベルのコメント、1-2 行） -->
  <div style="font-size:0.9em; color:#666; margin-bottom:16px; padding:8px 10px; background:#f8f8f8; border-left:3px solid #ccc; border-radius:3px;">{COMMENTARY}</div>
